...
function initGL (pass, multipass) {
  ...
  pass.render = function render (input, renderScreen) {
    ...
    debugger
    uniforms.uDepthTexture = multipass.getDepth(input, 1)
    // input.depth.bind(1)

    uniforms.uResolution = multipass.resolution
    uniforms.uResolutionRatio = multipass.resolution[1] / multipass.resolution[0]
...
function addGui (pass, gui) {
  ...
  folder.add(pass, 'depth', 0, 10).step(0.001)
  return folder
}
