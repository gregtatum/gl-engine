...
function drawPass (bindings, passIndex, passCount, outputFBO) {
  ...
  resolution[0] = width
  resolution[1] = height
  scale[0] = resizeResolutionX / width
  scale[1] = resizeResolutionY / height
  var scissorOffsetX = width * passIndex / passCount + width / passCount * MARGIN
  var scissorOffsetY = (height / 2) - (resizeResolutionY / 2)
  offset[0] = (width * (passIndex / passCount) + width / passCount * MARGIN) * 10/3
  offset[1] = height / 2 + height / passCount * 2
  ...
  gl.scissor(
    // coordinate of lower left of box
    scissorOffsetX,
    scissorOffsetY,
    resizeResolutionX,
    resizeResolutionY
...
export default function debug (gl) {
